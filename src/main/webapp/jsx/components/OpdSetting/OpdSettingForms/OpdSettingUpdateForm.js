...
import "react-widgets/dist/css/react-widgets.css";
import { useValidateOpdFormValuesHook } from "../../../formSchemas/opdSettingFormValidation";
...
import { useUpdateOpdSetting } from "../../../hooks/useUpdateOpdSetting";
...
const OpdSettingUpdateForm = (props) => {
  ...
  const { mutate, isLoading } = useUpdateOpdSetting(formik, props);
  const [currentFacId, setCurrentFacId] = useState(null)

  useEffect(async () => {
    if (history?.location?.state?.patientId) {
      const opdVisit = await fetchOpdSetting(history?.location?.state?.patientId)
...
export default OpdSettingUpdateForm;
