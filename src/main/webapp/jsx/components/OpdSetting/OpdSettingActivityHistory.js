...
import { fetchOpdSetting } from "../../services/fetchOpdSetting";
import { FETCH_ENROLMENT_KEY, FETCH_OPD_SETTING_KEY } from "../../utils/queryKeys";
import { useArchiveOpdSetting } from "../../hooks/useArchiveOpdSetting";
...
const OpdSettingActivityHistory = (props) => {
    ...
    for (let index = 0; index < array.length; index++) {
      ...
      if (activityRecord?.path === "hepatitis_followup") {
        queryClient.prefetchQuery(
          [FETCH_OPD_SETTING_KEY, activityRecord?.recordId],
          () => fetchOpdSetting(activityRecord?.recordId)
        );
      }
    }
  };
  ...
  const { mutate } = useArchiveOpdSetting(props, setSaving, toggle);
...
export default OpdSettingActivityHistory;
